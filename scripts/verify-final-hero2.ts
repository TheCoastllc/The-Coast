...
async function main() {
  ...
  // Dismiss cookie banner via Decline (privacy-preserving). It mounts late, so poll.
  let dismissed = false;
  for (let i = 0; i < 20 && !dismissed; i++) {
    try {
      const decline = page.getByRole('button', { name: /decline/i }).first();
      if (await decline.isVisible({ timeout: 1000 })) {
        await decline.click({ timeout: 2000 });
        dismissed = true;
        console.log(`cookie banner declined on attempt ${i}`);
      }
    } catch {}
    if (!dismissed) await page.waitForTimeout(1000);
  }
  if (!dismissed) console.log('cookie banner never appeared / not dismissed');
  ...
  async function scrollToY(target: number) {
    ...
    let iter = 0;
    while (Math.abs(y - target) > 8 && iter < 150) {
      const delta = target - y;
      const step = delta > 0 ? Math.min(800, delta) : Math.max(-800, delta);
      await page.mouse.wheel(0, step);
      await page.waitForTimeout(120);
      y = await page.evaluate(() => window.scrollY);
      iter++;
      if (target === 0 && y <= 0) break;
    }
    await page.waitForTimeout(700);
    return y;
  }

  // back to absolute top, then long settle so WebGL crossfade completes
  ...
  // zoom on sun region at p=0 (upper middle third), 2x
