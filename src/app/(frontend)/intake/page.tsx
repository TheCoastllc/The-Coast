...
export default function IntakePage() {
  ...
  // Load saved progress + prefill from GetStarted
  useEffect(() => {
    const saved = localStorage.getItem(STORAGE_KEY)
    if (saved) {
      try { setForm({ ...initial, ...JSON.parse(saved) }) } catch {}
    }
    ...
    if (prefill) {
      try {
        ...
        setForm((prev) => ({
          ...
          servicesInterested: data.services || prev.servicesInterested,
      ...
      <main className="min-h-screen bg-background">
        ...
        <div className="flex flex-col items-center justify-center min-h-[80vh] px-6 text-center">
          ...
          <p className="text-body text-muted-foreground text-lg max-w-md mb-8">
            Your intake form has been submitted. Expect a personalized response within 24 hours.
          </p>
          <button
            onClick={() => router.push('/')}
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input value={form.fullName} onChange={set('fullName')} placeholder="Jane Doe" className={inputClass} />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <input value={form.businessName} onChange={set('businessName')} placeholder="Your Business Name" className={inputClass} />
                  </div>
                  <div>
            ...
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-1.5">Describe your business *</label>
                    <textarea value={form.businessDescription} onChange={set('businessDescription')} placeholder="What does your business do? What problem do you solve?" rows={4} className={textareaClass} />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-1.5">Who is your ideal customer?</label>
                    <textarea value={form.idealCustomer} onChange={set('idealCustomer')} placeholder="Describe your target audience — age, interests, pain points..." rows={3} className={textareaClass} />
                  </div>
                </div>
              </motion.div>
            )}
              ...
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {SERVICES.map((service) => {
                    const isSelected = form.servicesInterested.includes(service)
                    return (
                      <button
                        key={service}
                        onClick={() => setForm((prev) => ({ ...prev, servicesInterested: toggleArr(prev.servicesInterested, service) }))}
                        className={`p-3 rounded-xl border text-sm text-left transition-all duration-200 flex items-center gap-2 ${
                          isSelected ? 'border-primary bg-primary/10 text-foreground' : 'border-border bg-card text-muted-foreground hover:border-primary/40'
                        }`}
                      >
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-2">Brand vibes (select all that apply)</label>
                    <div className="flex flex-wrap gap-2">
                      {BRAND_VIBES.map((vibe) => {
                        const isSelected = form.brandVibes.includes(vibe)
                        return (
                          <button
                            key={vibe}
                            onClick={() => setForm((prev) => ({ ...prev, brandVibes: toggleArr(prev.brandVibes, vibe) }))}
                            className={`px-3 py-1.5 rounded-full text-sm transition-colors ${
                              isSelected ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-muted-foreground hover:border-primary/50'
                            }`}
                          >
                            {vibe}
                          </button>
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-2">Color preferences</label>
                    <div className="flex flex-wrap gap-2">
                      {COLOR_PREFS.map((color) => {
                        const isSelected = form.colorPreferences.includes(color)
                        return (
                          <button
                            key={color}
                            onClick={() => setForm((prev) => ({ ...prev, colorPreferences: toggleArr(prev.colorPreferences, color) }))}
                            className={`px-3 py-1.5 rounded-full text-sm transition-colors ${
                              isSelected ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-muted-foreground hover:border-primary/50'
                            }`}
                          >
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-1.5">Colors to avoid</label>
                    <input value={form.colorsToAvoid} onChange={set('colorsToAvoid')} placeholder="Any colors you want to avoid?" className={inputClass} />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-1.5">Brands you admire</label>
                    <textarea value={form.brandsAdmired} onChange={set('brandsAdmired')} placeholder="List brands whose design you love and why..." rows={3} className={textareaClass} />
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {step === 4 && (
              <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                ...
                <div className="space-y-6">
                      ...
                      {BUDGETS.map((b) => (
                        <button
                          ...
                          className={`p-3 rounded-xl border text-sm text-left transition-all ${
                            form.budget === b ? 'border-primary bg-primary/10 text-foreground' : 'border-border bg-card text-muted-foreground hover:border-primary/40'
                          }`}
                        >
                          {b}
                        </button>
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-2">Timeline</label>
                    <div className="flex flex-wrap gap-2">
                      {TIMELINES.map((t) => (
                        <button
                          key={t}
                          ...
                          className={`px-3 py-1.5 rounded-full text-sm transition-colors ${
                            form.timeline === t ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-muted-foreground hover:border-primary/50'
                          }`}
                        >
                          {t}
                        </button>
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground block mb-1.5">Anything else you&apos;d like us to know?</label>
                    <textarea value={form.additionalVision} onChange={set('additionalVision')} placeholder="Any additional context, goals, or notes for our team..." rows={4} className={textareaClass} />
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <div className="flex items-center justify-between mt-10">
            ...
            {step < steps.length - 1 ? (
              ...
              <button
                ...
                disabled={isSubmitting || !form.fullName || !form.email || !form.businessName}
