                  ...
                  <div className="aspect-[16/9] md:aspect-[21/9] relative bg-muted">
                    ...
                    <div className="absolute top-4 right-4 bg-background/80 backdrop-blur-sm px-3 py-1.5 rounded-full">
                      <span className="text-xs text-muted-foreground font-medium">
                        Conceptual Rendering – Not Actual Building
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
            {conceptImages.map((_, index) => (
              <button
                ...
                className="w-2 h-2 rounded-full bg-foreground/30 hover:bg-foreground/60 transition-colors"
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          >
            ...
            <p className="text-body text-muted-foreground text-lg md:text-xl max-w-2xl mx-auto">
              These conceptual renderings showcase a futuristic waterfront headquarters, blending innovation with
              coastal serenity. <strong className="text-foreground">Not actual buildings</strong>—pure inspiration for
            ...
            {conceptImages.map((image, index) => (
              ...
              >
                ...
                <div className="flex-1 space-y-4">
                  <h2 className="text-heading text-2xl md:text-3xl flex items-center gap-3">
                    <Building2 className="w-6 h-6 text-primary" />
                    {image.title}
                  </h2>
                  <p className="text-body text-muted-foreground leading-relaxed">{image.description}</p>
                  <p className="text-sm text-muted-foreground/70 italic">
                    *This is a conceptual visualization only and does not represent an actual building or construction plans.
                  </p>
                </div>
              </motion.div>
            ))}
          ...
          <motion.div
            ...
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            className="mt-20 md:mt-32 bg-card rounded-2xl p-8 md:p-12 border border-border"
          >
            ...
            <div className="space-y-6 text-body text-muted-foreground leading-relaxed max-w-2xl mx-auto text-center">
              <p>
                These visionary concepts embody everything The Coast stands for:
                <strong className="text-foreground"> innovation, coastal serenity, and the bold pursuit of excellence</strong>.
              </p>
              <p>
                Just as we help small businesses transform their visions into memorable brands, these renderings
                represent our own dream—a future headquarters that serves as a beacon for creative minds and ambitious
                entrepreneurs.
              </p>
              <p className="text-sm italic text-muted-foreground/70">
                Note: These are purely conceptual designs created for inspiration and do not represent actual
                construction plans or real estate developments.
              </p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/brand-avatar"
                className="group inline-flex items-center gap-3 px-8 py-4 bg-primary text-primary-foreground rounded-full text-mono text-sm hover:bg-primary/90 transition-colors duration-300"
              >
                Start with Brand Builder
            ...
            <p className="mt-8 text-sm text-muted-foreground">
              Inspired by our HQ Vision? Let us help you create a brand that&apos;s just as unforgettable.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <div className="fixed bottom-4 right-4 hidden lg:block">
        <div className="bg-card/80 backdrop-blur-sm border border-border rounded-lg px-4 py-3 max-w-xs shadow-lg">
          <p className="text-xs text-muted-foreground">
            <strong className="text-foreground">Conceptual only</strong> – imagining the future of branding.
          </p>
        </div>
      </div>
