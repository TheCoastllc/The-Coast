...
import { LEAVE_REVIEW_URL } from '@/lib/google-reviews'
...
function ReviewCards({ reviews }: { reviews: GoogleReview[] }) {
  return (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
      {reviews.map((review, i) => (
        <div
          key={review.reviewId ?? `review-${i}`}
          className="bg-white/3 border border-white/10 rounded-xl p-6 hover:border-primary/30 transition-all duration-500"
        >
          <div className="flex gap-1 mb-4" aria-label={`${review.starRating} out of 5 stars`}>
...
export function GoogleReviewsClient({ reviews }: { reviews: GoogleReview[] }) {
  const hasServerReviews = reviews.length > 0

  return (
    <section className="py-32 bg-black" aria-labelledby="reviews-heading">
      <div className="max-w-6xl mx-auto px-6">
        ...
        {hasServerReviews ? (
          // Render reviews directly so crawlers + no-JS users see real content,
          // not the "Loading reviews…" state from the react-google-reviews wrapper.
          <ReviewCards reviews={reviews} />
        ) : (
          // Last-resort fallback: client-only widget. Only reached when the
          // server-side Featurable fetch returned zero (transient outage).
          <ReactGoogleReviews
            layout="custom"
